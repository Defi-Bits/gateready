...
const venture = arg("venture", "gateready");
...
const steps = [
  ...
  "node scripts/content-from-site.mjs",
  "node scripts/content-to-twitter.mjs",
  `npm run -s outreach:publish:live -- --mode ${mode} --channel twitter --max 2 --delayMs 3500`,
  "node scripts/content-queue-view.mjs"
];

const results = steps.map((s) => ({ step: s, ...run(s) }));
const failed = results.filter((r) => !r.ok).length;
console.log(JSON.stringify({ ok: failed === 0, failed, mode, venture, modeReason: mode === "dryrun" ? "enforced_by_structure" : "requested", results }, null, 2));
